<h3>Составление расписания работ на <?=$date;?>&nbsp;&nbsp;&nbsp;
<a href="#" class="btn btn-small btn-warning pull-right" style="margin-right:25px;" id="iNeedHelp">Помощь</a></h3>
<table class="table table-bordered table-condensed" id="shed" style="margin-right:20px;">
	<!-- <tr>
		<th>Часы</th><?=$headings1;?>
	</tr> -->
	<tr>
		<th>Время</th><?=$headings2;?>
	</tr>
	<!-- <tr>
...
<div class="modal hide" id="ifYesModal">
	...
	<div class="modal-body">
		...
		<div id="instructor_info">&nbsp;</div>
		<label><input type="checkbox" id="useTNext">&nbsp;&nbsp;Назначить на следующую процедуру</label>
		<label><input type="checkbox" id="useTAlways">&nbsp;&nbsp;Назначить на весь курс</label>
		<center>
...
<div class="modal hide" id="helpM">
	...
	<div class="modal-body">
		В столбцах расположены пациенты и прописанные им на указанную дату услуги и процедуры.<br><br>Щелчок на клетки столбца позволяет выбрать начало оказания услуги по графику, а повторный щелчок - позволяет перейти к выбору инструктора и затем подтвердить выбор. <br>Это назначит услугу в персональный график инструктора.
		<center>
			<button class="btn ifYes" value="0" data-dismiss="modal" aria-hidden="true">Закрыть</button>
		</center>
	</div>
...
<!--
	...
	$(".serviceCell").unbind('click').click(function(){
		var staff,
		...
		if ( !isSelected ) {
			...
			$(this).addClass('servselected').html('<img src="<?=base_url();?>images/question-button.png" width="16" height="16" border="0" alt="">');
	...
	function ifYes(column, starttime, stoptime, fio, title, initTime, endTime){
		...
		$(".ifYes").unbind('click').click(function(){
			if ( parseInt($(this).val() ) == 1 ) {
				...
				$.ajax({
					url        : "<?=base_url();?>shedule/savetime",
					type       : "POST",
					data       : {
						cal    : column,
						start  : starttime + ":00",
						stop   : stoptime  + ":00",
						next   : ($("#useTNext").attr('checked') == 'checked')   ? 1 : 0,
						all    : ($("#useTAlways").attr('checked') == 'checked') ? 1 : 0,
						inst   : $("#instructor").val()
					},
					dataType   : 'text',
					success    : function(data) {
						if(data == "OK"){
							...
							$('.c' + column).each(function(){
								a = parseInt($(this).attr('ref'));
								if(a >= initTime && a <= endTime){
									$(this).addClass("servactive").html('<img src="<?=base_url();?>images/alarm-clock.png" width="16" height="16" border="0" alt="">');
								}
							});
							manageStaff();
						}
			...
			$('.c' + column).removeClass('servselected');
			$('.c' + column).filter(".servactive").html('<img src="<?=base_url();?>images/alarm-clock.png" width="16" height="16" border="0" alt="">')
		});
	}
